...
#define USE_NEON_RATE 1
#define NO_USE_NEON_RATE 1
#define ZERO_USE_NEON_RATE 0
...
void pp_add(fp12_t l, ep2_t r, ep2_t q, ep_t p) {
	...
	TRY{
		...
		dv2_new(T1);
#if USE_NEON_RATE
		fp2_mul_dual(t1, t2, r->z, q->x, r->z, q->y);
#else
		fp2_mul(t1, r->z, q->x);
		fp2_mul(t2, r->z, q->y);
#endif
		...
		fp2_sqr(t3, t1);
#if USE_NEON_RATE
		fp2_mul_dual(t4, r->x, t2, t2, t3, r->x);
#else		
		fp2_mul(r->x, t3, r->x);
		fp2_sqr(t4, t2);
#endif
...
#if USE_NEON_RATE
		fp2_mul_dual(t3, t4, t1, t3, t4, r->z);
#else
		fp2_mul(t3, t1, t3);
		fp2_mul(t4, t4, r->z);
#endif
		fp2_add(t4, t3, t4);
#if USE_NEON_RATE
		fp_mul_256_dual_NEON(l[1][0][0], l[1][0][1], t2[0], p->x, t2[1], p->x);
#else
		...
		fp2_sub(r->x, r->x, t4);
#if USE_NEON_RATE
		fp2_mul_dual_nr(T0, T1, t2, r->x, t3, r->y);
#else
...
#if USE_NEON_RATE
		fp2_mul_dual(r->x, r->z, t1, t4, r->z, t3);
		fp2_mul_dual_nr(T0, T1, q->x, t2, q->y, t1);
#else
		fp2_mul(r->x, t1, t4);
		fp2_mul(r->z, r->z, t3);
...
#if USE_NEON_RATE
		fp2_rdc_NEON(l[1][1][0], l[1][1][1], T0[0], T0[1]);
		fp_mul_256_dual_NEON(l[0][0][0], l[0][0][1], t1[0], p->y, t1[1], p->y);
#else
		fp2_rdc(l[1][1], T0);
		// l01 = t1 * yp. 
...
void pp_dbl(fp12_t l, ep2_t r, ep2_t q, ep_t p) {
	...
	TRY{
...
#if ZERO_USE_NEON_RATE
		fp2_mul_dual(t0, t1, q->x, q->x, q->y, q->y);
#else
		...
		fp2_add(t4, q->x, q->y);
#if ZERO_USE_NEON_RATE
		fp2_mul_dual(t2, t4, q->z, q->z, t4, t4);
#else
...
#else
#if USE_NEON_RATE
		fp2_mul_dual(t2, t3, q->z, q->z, q->x, q->y);
#else
		fp2_sqr(t2, q->z); // t2 = z1^2
		...
		fp2_subd(T0, T0, T1);
		//fp2_rdc(r->y, T0);		
		fp2_rdc_NEON(r->y[0], r->y[1], T0[0], T0[1]);
...
#if USE_NEON_RATE
		fp_mul_256_dual_NEON(l[1][0][0], l[1][0][1], l[1][0][0], p->x, l[1][0][1], p->x);
		fp_mul_256_dual_NEON(l[0][0][0], l[0][0][1], t5[0], p->y, t5[1], p->y);
#else
...
//
		fp2_div2(t2, t4); // t2 = (y1^2 + 9b.z1^2)/2
#if ZERO_USE_NEON_RATE
		fp2_mul_dual(t2, t4, t2, t2, t3, t3);
#else
...
#if USE_NEON_RATE
		fp_mul_256_dual_NEON(l[1][0][0], l[1][0][1], l[1][0][0], p->x, l[1][0][1], p->x);
		fp_mul_256_dual_NEON(l[0][0][0], l[0][0][1], t2[0], p->y, t2[1], p->y);
#else
		fp_mul(l[1][0][0], l[1][0][0], p->x);
		fp_mul(l[1][0][1], l[1][0][1], p->x); // l02 = (3x1^2)xp
...
void pp_addm(fp12_t l, ep2_t r, ep2_t q, ep_t p) {
	...
	TRY{
		...
		fp2_add(t4, t3, t4);
#if USE_NEON_RATE
		fp_mul_256_dual_NEON(l[0][1][0], l[0][1][1],  t2[0], p->x, t2[1], p->x);
#else
...
#if USE_NEON_RATE
		fp2_rdc_NEON(l[0][0][0], l[0][0][1], T0[0], T0[1]);
		fp_mul_256_dual_NEON(l[1][1][0], l[1][1][1], t1[0], p->y, t1[1], p->y);
#else		
		fp2_rdc(l[0][0], T0);
		// l01 = t1 * yp. 
...
#if USE_NEON_RATE
		fp_mul_256_dual_NEON(l[0][1][0], l[0][1][1], l[0][1][0], p->x, l[0][1][1], p->x);
		fp_mul_256_dual_NEON(l[1][1][0], l[1][1][1], t2[0], p->y, t2[1], p->y);
#else
