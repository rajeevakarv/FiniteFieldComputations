...
#define NO_USE_NEON_FP12 1
#define USE_NEON_FP12 1
...
*/
void fp6_rdc_temp(fp6_t C, dv6_t A){
#if USE_NEON_FP12
	fp2_rdc_NEON(C[0][0], C[0][1], A[0][0], A[0][1]);
	fp2_rdc_NEON(C[1][0], C[1][1], A[1][0], A[1][1]);
	fp2_rdc_NEON(C[2][0], C[2][1], A[2][0], A[2][1]);
#else
	fp6_rdc(C,A);
#endif
}
static void fp4_sqr(fp2_t c, fp2_t d, fp2_t a, fp2_t b) {
	...
	TRY {
		// t0 = a^2. 
	      ...
	      fp2_addd(T2, T2, T0);
#if USE_NEON_FP12
		  fp2_rdc_NEON(c[0], c[1], T2[0], T2[1]);
#else
		  fp2_rdc(c, T2);
#endif
	      ...
	      fp2_subd(T0, T0, T1);
#if USE_NEON_FP12 
		 fp2_rdc_NEON(d[0], d[1], T0[0], T0[1]);
#else
		 fp2_rdc(d, T0);
#endif
...
void fp12_mul_sxs(fp12_t c, fp12_t a, fp12_t b) {
	...
	fp2_null(v7);
		...
		fp2_new(v7);
#if NO_USE_NEON_FP12
		fp2_mul_dual(v0, v1, a[0][0], b[0][0],a[1][0], b[1][0]);
#else		
...
void fp12_mul_dxsm(fp12_t c, fp12_t a, fp12_t b) {
	...
	TRY {
		...
		fp6_add(v0, a[0], a[1]);
		fp2_copy(v1[0], b[0][0]);
		fp2_add(v1[1], b[0][1], b[1][1]);
		fp6_mul_dxs_nr(T0, v0, v1);
		//v0=a0b0
		fp6_mul_dxs_nr(V0, a[0], b[0]);
		// v1 = a1b1 
		fp6_mul_dxq2_nr(V1, a[1], b[1][1]);
...
void fp12_mul_sxsm(fp12_t c, fp12_t a, fp12_t b) {
	...
	fp2_null(v7);
		...
		fp2_new(v7);
#if NO_USE_NEON_FP12
		fp2_mul_dual(v0, v1, a[0][0], b[0][0]);
#else
		fp2_mul(v0, a[0][0], b[0][0]);
...
void fp12_frb(fp12_t c, fp12_t a) {
	...
	fp2_mul_frb_dual(c[1][0], c[0][1], c[1][0], 1, c[0][1], 2);
	fp2_mul_frb_dual(c[1][1], c[0][2], c[1][1], 3, c[0][2], 4);
...
// When using new compressed squarings
	...
	TRY {
		...
		switch (fp_param_get()){
			...
			fp12_conv_back_sim(t1, t0, t1, t0);
			fp12_conv_back(t2, t2);
			fp12_conv_back_simt(t2, t1, t0, t2, t1, t0);
			fp12_inv_uni(t1, t1);
			fp12_inv_uni(t0, t0);
			fp12_mul(c, t0, a);
			fp12_mul(c, c, t1);
...
// New compressed squaring using lazy reduction 
       ...
       fp2_subd(T3, T2, T3);
#if USE_NEON_FP12
		fp2_rdc_NEON(t0[0], t0[1], T3[0], T3[1]);
#else
       fp2_rdc(t0, T3);
#endif
       ...
       fp2_addd(T4, T0, T4);
#if USE_NEON_FP12
	   fp2_rdc_NEON(t0[0], t0[1], T4[0], T4[1]);
#else
	   fp2_rdc(t0, T4);
#endif
...
void fp12_conv_back(fp12_t c, fp12_t a) {
	...
	TRY {
		...
		// t0 = g1. 
#if NO_USE_NEON_FP12
		fp2_mul_dual(c[1][1], t3, t0, t1, a[0][2], a[0][1]);
#else
		fp2_mul(c[1][1], t0, t1);
		
		// t1 = g3 * g4. 
		fp2_mul(t3, a[0][2], a[0][1]);
#endif
		// t2 = 2 * g1^2 - 3 * g3 * g4. 
		fp2_sqr(t2, c[1][1]);
		fp2_sub(t2, t2, t3);
...
void fp12_conv_back_sim(fp12_t c1, fp12_t c0, fp12_t a1, fp12_t a0) {
	...
	TRY {
		...
		fp2_inv(t4, t4);
#if USE_NEON_FP12
		fp2_mul_dual(t1, t3, t4, a1[1][0], t4, a0[1][0]);
		fp2_mul_dual(c0[1][1], c1[1][1], t0, t1, t2, t3);
		fp2_mul_dual(t1, t5, a0[0][2], a0[0][1], a0[1][0], a0[1][2]);
#else
...
#if USE_NEON_FP12
		fp2_mul_dual(t5, t1, a1[0][2], a1[0][1], a1[1][0], a1[1][2]);
#else		
		/* t1 = g3 * g4. */
		fp2_mul(t5, a1[0][2], a1[0][1]);
		/* t1 = g2 * g5. */
...
void fp12_conv_back_simt(fp12_t c2, fp12_t c1, fp12_t c0, fp12_t a2, fp12_t a1, fp12_t a0) {
        fp2_t t0, t1, t2, t3, t4, t5, t6, t7, t8;
	...
	fp2_null(t8);
		...
		fp2_mul(t7, a0[1][0], a1[1][0]);
		fp2_mul(t4, t7, a2[1][0]);
		fp2_dbl(t4, t4);
		fp2_dbl(t4, t4);
		
		fp2_inv(t4, t4);
#if NO_USE_NEON_FP12
		fp2_mul_dual(t6, t8, t4, t7, a1[1][0], a2[1][0]);
		fp2_mul_dual(t1, t7, t4, t8, a0[1][0], a2[1][0]);
#else
		fp2_mul(t6, t4, t7);
		fp2_mul(t8, a1[1][0], a2[1][0]);
		fp2_mul(t1, t4, t8);
		fp2_mul(t7, a0[1][0], a2[1][0]);
#endif
		
#if NO_USE_NEON_FP12
		fp2_mul_dual(t3, c0[1][1], t4, t7, t0, t1);
		fp2_mul_dual(c1[1][1], c2[1][1], t2, t3, t5, t6);
#else
		fp2_mul(t3, t4, t7);
		/* t0 = g1. */
		fp2_mul(c0[1][1], t0, t1);
		...
		fp2_mul(c2[1][1], t5, t6);
#endif

#if USE_NEON_FP12
		fp2_mul_dual(t8, t1, a0[0][2], a0[0][1], a0[1][0], a0[1][2]);
#else
		/* t1 = g3 * g4. */
		fp2_mul(t8, a0[0][2], a0[0][1]);
		/* t1 = g2 * g5. */
...
#if USE_NEON_FP12
		fp2_mul_dual(t1, t8, a1[1][0], a1[1][2], a2[0][2], a2[0][1]);
#else
		/* t1 = g2 * g5. */
		fp2_mul(t1, a1[1][0], a1[1][2]);
		/* t1 = g3 * g4. */
		fp2_mul(t8, a1[0][2], a1[0][1]);
#endif
		/* t2 = E * (2 * g1^2 + g2 * g5 - 3 * g3 * g4) + 1. */
		...
		fp2_sub(t2, t2, t8);
#if USE_NEON_FP12
		fp2_mul_dual(t8, t4, a2[0][2], a2[0][1], a2[1][0], a2[1][2]);
		
#else
		/* t1 = g3 * g4. */
		fp2_mul(t8, a2[0][2], a2[0][1]);
		/* t1 = g2 * g5. */
		fp2_mul(t4, a2[1][0], a2[1][2]);
#endif
		/* t2 = E * (2 * g1^2 + g2 * g5 - 3 * g3 * g4) + 1. */
		fp2_add(t2, t2, t1);
		fp2_mul_nor(c1[0][0], t2);
		...
		/* t2 = E * (2 * g1^2 + g2 * g5 - 3 * g3 * g4) + 1. */
		fp2_add(t5, t5, t4);
		fp2_mul_nor(c2[0][0], t5);
		fp_add_dig(c2[0][0][0], c2[0][0][0], 1);
