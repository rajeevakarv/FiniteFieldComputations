...
static void ep2_add_projc_impl(ep2_t r, fp2_t s, ep2_t p, ep2_t q) {
	...
	TRY {
		...
		fp2_sqr(t1, p->z);
#if USE_NEON_EP2
		fp2_mul_dual(t3, t5, t3, t1, t1, p->z);
#else		
		fp2_mul(t3, t3, t1);
		fp2_mul(t5, t1, p->z);
#endif		
/*		fp2_mul(t4, t4, t5);
		...
		fp2_sub(t3, t3, p->x);
#if USE_NEON_EP2		
		fp2_mul_dual(t5, r->z, t4, t5, p->z, t3);
#else		
		fp2_mul(t5, t4, t5);
		fp2_mul(r->z, p->z, t3);
#endif		
		...
		fp2_sqr(t1, t3);
#if USE_NEON_EP2
		fp2_mul_dual(t4, t5, t1, t3, t1, p->x);
#else
		fp2_mul(t4, t1, t3);
		fp2_mul(t5, t1, p->x);
#endif
		...
		fp2_sub(t1, t5, r->x);
#if USE_NEON_EP2
		fp2_mul_dual(t1, t4, t1, t0, t4, p->y);
#else		
		fp2_mul(t1, t1, t0);
		fp2_mul(t4, t4, p->y);
#endif		
...
static void ep2_dbl_projc_impl(ep2_t r, fp2_t s, fp2_t e, ep2_t p) {
	...
	TRY {
		...
		fp2_new(t3);
#if USE_NEON_EP2
		fp2_mul_dual(t0, t3, p->x, p->x, p->y, p->y);
#else		
		fp2_sqr(t0, p->x);
		fp2_sqr(t3, p->y);
#endif		
...
#if USE_NEON_EP2
		fp2_mul_dual(t1, r->z, t3, p->x, p->z, r->z);
#else
		fp2_mul(t1, t3, p->x);
		fp2_mul(r->z, p->z, p->y);
#endif	
...
void ep2_norm2(ep2_t r, ep2_t p) {
	if (!p->norm) {
		...
		TRY {
			...
			fp2_inv(t1, p->z);
#if USE_NEON_EP2
			fp2_mul_dual(r->x, r->y, p->x, t1, p->y, t1);
#else			
			fp2_mul(r->x, p->x, t1);
			fp2_mul(r->y, p->y, t1);
#endif			
...
void ep2_norm_impl(ep2_t r, ep2_t p) {
	if (!p->norm) {
		...
		TRY {
			...
			fp2_sqr(t0, t1);
#if USE_NEON_EP2
			fp2_mul_dual(r->x, r->y, p->x, t0, p->y, t0);
#else
			fp2_mul(r->x, p->x, t0);
			fp2_mul(r->y, p->y, t0);
#endif			
...
void ep2_frb(ep2_t r, ep2_t p) {
	...
	fp2_mul_frb_dual(r->x, r->y, r->x, 2, r->y, 3);
...
void ep2_frbm(ep2_t r, ep2_t p) {
	...
	fp2_mul_frb_dual(r->x, r->y, p->x, 8, p->y, 8);
	fp2_frb(r->x, r->x);
	...
	fp2_mul_frb_dual(r->x, r->y, r->x, 6, r->y, 7);
