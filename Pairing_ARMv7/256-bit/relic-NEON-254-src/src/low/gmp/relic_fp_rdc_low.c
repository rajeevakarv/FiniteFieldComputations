 ...
 *
 * @version $Id: relic_fp_rdc_low.c 182 2009-11-17 19:37:43Z dfaranha $
 * @ingroup fp
 */
...
dig_t fp_rdcn_low2(dig_t *c, dig_t *a, dig_t *m, dig_t u) {
	int i;
	dig_t r, carry, *tmp;
	...
	for (i = 0; i < FP_DIGS; i++, tmp++) {
		...
		carry = fp_muladd_low(tmp, m, r);
		carry = mpn_add_1(tmp + FP_DIGS, tmp + FP_DIGS, FP_DIGS - i, carry); // todo
	}
