...
#define USE_NEON_FP6 1
#define NO_USE_NEON_FP6 1    //it was zero.
typedef dig_t Fp[9];    //extra digit to hold carry.
typedef Fp Fp2[2]; 
/*============================================================================*/
/* Public definitions                                                         */
/*============================================================================*/
...
// Sextic extension field multiplication using Karatsuba and lazy reduction
	...
	dv2_t r0, r1, r2, au, bu, cu;
	 ...
	 TRY{
		...
		fp2_add(s1, b[1], b[2]);
#if USE_NEON_FP6
		fp2_mul_dual_nr(au, r1, s1,s0, a[1], b[1]);
#else
		fp2_mul_nr(au, s1, s0);
		fp2_mul_nr(r1, a[1], b[1]);
#endif
	...
	//C1
		...
		fp2_subd(au, au, r1);
#if USE_NEON_FP6
		fp2_mul_dual_nr(bu, r2, s1, s0, a[2], b[2]);
#else
		fp2_mul_nr(bu, s1, s0);
		fp2_mul_nr(r2, a[2], b[2]);
#endif	
		fp2_subd(au, au, r2);
		fp2_mul_nord(C[0], au); 
		...
		fp2_add(s1, b[0], b[2]);
#if USE_NEON_FP6
		fp2_mul_dual_nr(au, r0, s1, s0, a[0], b[0]);
#else
		fp2_mul_nr(au, s1,s0);
		fp2_mul_nr(r0, a[0], b[0]);
#endif
		fp2_addd(C[0], C[0], r0);
...
// "Simplified" sextic extension field multiplication using Karatsuba and lazy reduction
  fp2_t t0, t1, t2, t3;
                ...
                TRY {
					   ...
					   dv2_new(T3);
#if USE_NEON_FP6
		fp2_mul_dual_nr(T0, T1, a[0], b[0], a[1], b[1]);
#else
                       // v0 = a0b0
                       fp2_mul_nr(T0, a[0], b[0]);
                       ...
                       fp2_addd(T2, T2, T0);
                       //fp2_rdc(t2, T2);
						fp2_rdc_NEON(t2[0], t2[1], T2[0], T2[1]);
					   ...
					   fp2_add(t3, a[0], a[2]);
#if USE_NEON_FP6
					fp2_mul_dual_nr(T2, T3, t0, t1, t3, b[0]);
#else
                       fp2_mul_nr(T2, t0, t1);
					   fp2_mul_nr(T3, t3, b[0]);
#endif                       
					   fp2_subd(T2, T2, T0);
                       fp2_subd(T2, T2, T1);
                       //fp2_rdc(c[1], T2);
						fp2_rdc_NEON(c[1][0], c[1][1], T2[0], T2[1]);
						
						fp2_subd(T3, T3, T0);
                       fp2_addd(T3, T3, T1);
                       //fp2_rdc(c[2], T3);
						fp2_rdc_NEON(c[2][0], c[2][1], T3[0], T3[1]);
...
// "Simplified" sextic extension field multiplication w/o reduction using Karatsuba and lazy reduction
	  ...
	  TRY {
	    ...
	    dv2_new(T2);
#if USE_NEON_FP6
		fp2_mul_dual_nr(T0, T2, a[0], b[0], a[2], b[1]);
#else
	    // v0 = a0b0
	    fp2_mul_nr(T0, a[0], b[0]);
	    ...
	    fp2_add(t1, b[0], b[1]);
#if USE_NEON_FP6
		fp2_mul_dual_nr(C[1], T1, t0, t1, a[1], b[1]);
#else
	    fp2_mul_nr(C[1], t0, t1);
	    fp2_mul_nr(T1, a[1], b[1]);
#endif
...
#if USE_NEON_FP6
	fp2_mul_dual(c[0], c[1], a[0], b, a[1], b);
	fp2_mul(c[2], a[2], b);
#else
	fp2_mul(c[0], a[0], b);
	fp2_mul(c[1], a[1], b);
...
void fp6_mul_dxq_nr(dv6_t C, fp6_t a, fp2_t b) {
 #if USE_NEON_FP6
	fp2_mul_dual_nr(C[0], C[1], a[0], b, a[1], b);
...
#if USE_NEON_FP6
	fp_mul_256_dual_NEON(c[0][0], c[0][1], a[0][0], b, a[0][1], b);
	fp_mul_256_dual_NEON(c[1][0], c[1][1], a[1][0], b, a[1][1], b);
	fp_mul_256_dual_NEON(c[2][0], c[2][1], a[2][0], b, a[2][1], b);
#else
...
#if USE_NEON_FP6
	fp_mul_256_dual_NEON_nr(C[0][0], C[0][1], a[0][0], b, a[0][1], b);
	fp_mul_256_dual_NEON_nr(C[1][0], C[1][1], a[1][0], b, a[1][1], b);
	fp_mul_256_dual_NEON_nr(C[2][0], C[2][1], a[2][0], b, a[2][1], b);
#else
...
#if USE_NEON_FP6
	fp2_mul_dual_nr(C[1], C[2], a[0], b, a[1], b);
#else
...
#if USE_NEON_FP6  
  fp_mul_256_dual_NEON_nr(C[1][0], C[1][1], a[0][0], b, a[0][1], b);
  fp_mul_256_dual_NEON_nr(C[2][0], C[2][1], a[1][0], b, a[1][1], b);
  fp_mul_256_dual_NEON_nr(C[0][0], C[0][1], a[2][0], b, a[2][1], b);
#else
...
void fp6_sqr(fp6_t c, fp6_t a) {
	...
	TRY {
		...
		fp2_sqr(t0, a[0]);
#if USE_NEON_FP6
		fp2_mul_dual(t1, t3, a[0], a[1], a[1], a[2]);
#else
		// t1 = 2 * a0 * a1 
		fp2_mul(t1, a[0], a[1]);
		// t3 = 2 * a1 * a2 
...
dv2_null(T4);
	...
	TRY {
		...
		fp2_sqr_nr(T0, a[0]);
#if USE_NEON_FP6
		fp2_mul_dual_nr(T1, T3, a[0], a[1], a[1], a[2]);
#else
...
#else
	fp2_mul_frb_dual(c[1], c[2], c[1], 2, c[2], 4);
#endif
}
...
void fp6_inv(fp6_t c, fp6_t a) {
	...
	TRY {
		...
		fp2_mul_dual_nr(V0, V1, a[1], a[2], a[0], a[1]);
		fp2_mul_nord(V0, V0);
		fp2_subd(V0, T0, V0);
#if USE_NEON_FP6
		fp2_rdc_NEON(v0[0], v0[1], V0[0], V0[1]);
#else		
		fp2_rdc(v0, V0);
#endif		
		...
		fp2_subd(V1, T0, V1);
#if USE_NEON_FP6
		fp2_rdc_NEON(v1[0], v1[1], V1[0], V1[1]);
#else
		fp2_rdc(v1, V1);
#endif		
		...
		fp2_subd(V2, T0, V2);
#if USE_NEON_FP6
		fp2_rdc_NEON(v2[0], v2[1], V2[0], V2[1]);		
#else		
		fp2_rdc(v2, V2);
#endif
		
#if NO_USE_NEON_FP6
		fp2_mul_dual(c[1], c[0], a[1], v2, a[0], v0);
#else
		fp2_mul(c[1], a[1], v2);
		fp2_mul(c[0], a[0], v0);
#endif		
		...
		fp2_inv(t0, t0);
#if NO_USE_NEON_FP6		
		fp2_mul_dual(c[0], c[1], v0, t0, v1, t0);
#else		
		fp2_mul(c[0], v0, t0);
		fp2_mul(c[1], v1, t0);
#endif		
