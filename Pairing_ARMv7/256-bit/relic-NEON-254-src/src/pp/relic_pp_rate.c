...
#define USE_NEON 1    //it was zero
#define USE_NEON_TEMP 1
/*============================================================================*/
/* Private definitions                                                        */
/*============================================================================*/
...
void pp_add(fp12_t l, ep2_t r, ep2_t q, ep_t p) {
	...
	TRY{
		...
		fp2_sub(t0, q->y, r->y);
		fp2_sub(t1, q->x, r->x);
		fp2_inv(t2, t1); //t1 = 1/2y
		fp2_mul(t2, t2, t0); //t2=3x^2/2y
		...
		//fp2_mul(t6, t2, q->x);	
		//fp2_mul(t0, t2, t5);
		fp2_mul_dual(t6, t0, t2, q->x, t2, t5);
		fp2_sub(l[1][1], t6, q->y); // t3 = 3b.z1^2 - y1^2

		fp2_copy(r->x, t1);
		fp2_sub(r->y, t0, r->y);
#else
		fp2_mul(t4, t2, q->x);	
		fp2_sub(l[1][1], t4, q->y); // t3 = 3b.z1^2 - y1^2
		...
		//fp_mul(l[1][0][1], t2[1], p->x); // l02 = (3x1^2)xp
...
void pp_dbl(fp12_t l, ep2_t r, ep2_t q, ep_t p) {
	...
	TRY{
		...
		fp2_add(t0, q->y, q->y);
		fp2_inv(t1, t0); //t1 = 1/2y
		fp2_sqr(t0, q->x);
		fp2_add(t2, t0, t0); //t2=2x^2
		fp2_add(t2, t2, t0); //t2=3x^2
		fp2_mul(t2, t2, t1); //t2=3x^2/2y
...
void pp_dbl(fp12_t l, ep2_t r, ep2_t q, ep_t p) {
	...
	TRY{
		...
		//fp2_mul(t4, t2, q->x);
		//fp2_mul(t0, t2, t5);
		fp2_mul_dual(t4, t0, t2, q->x, t2, t5);
		fp2_sub(l[1][1], t4, q->y); // t3 = 3b.z1^2 - y1^2
...
void pp_addm(fp12_t l, ep2_t r, ep2_t q, ep_t p) {
	...
	TRY{
		...
		//fp2_mul(t0, t2, t5);		
		fp2_mul_dual(t4, t0, t2, q->x, t2, t5);
		fp2_sub(l[0][0], t4, q->y); // t3 = 3b.z1^2 - y1^2
#else
		fp2_mul(t4, t2, q->x);	
		fp2_sub(l[0][0], t4, q->y); // t3 = 3b.z1^2 - y1^2
		...
		//fp_mul(l[0][1][1], t2[1], p->x); // l02 = (3x1^2)xp
...
void pp_dblm(fp12_t l, ep2_t r, ep2_t q, ep_t p) {
	...
	TRY{
		...
		fp2_sub(t5, q->x, t1);
		fp2_mul_dual(t6, t0, t2, q->x,t2, t5);
		fp2_sub(l[0][0], t6, q->y); // t3 = 3b.z1^2 - y1^2
...
void pp_miller(fp12_t res, ep2_t t, ep2_t q, bn_t r, ep_t p) {
	...
	TRY {
		...
		fp_zero(res[0][0][1]);
		fp_copy(res[0][0][0], p->y);
		fp_zero(tmp[0][0][1]);
		fp_copy(tmp[0][0][0], p->y);
		ep2_copy(t, q);
		pp_dbl(res, t, t, pn);
...
void pp_millerm(fp12_t res, ep2_t t, ep2_t q, bn_t r, ep_t p) {
	...
	TRY {
		...
		fp_neg(pn->x, pn->x);
		fp_zero(res[1][1][1]);
		fp_copy(res[1][1][0], p->y);
		fp_zero(tmp[1][1][1]);
		fp_copy(tmp[1][1][0], p->y);
		ep2_copy(t, q);
		pp_dblm(res, t, t, pn);
